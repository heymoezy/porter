 ...
 *
 * All queries use async PostgreSQL via pg.ts helpers + brain pool.
 */
import { queryOne, queryAll, execute } from '../db/pg.js';
import { brain } from '../db/client.js';
...
async function runTrainer(item: PipelineItem): Promise<void> {
  ...
  try {
    // Read template skills (PG jsonb auto-parses -- no JSON.parse needed)
    ...
    // Map to persona_skills (upsert via WHERE NOT EXISTS)
...
async function runOutfitter(item: PipelineItem): Promise<void> {
  ...
  try {
    // Read template tools (PG jsonb auto-parses -- no JSON.parse needed)
...
async function resetPipeline(): Promise<void> {
  await stop();
  const client = await brain.connect();
  try {
