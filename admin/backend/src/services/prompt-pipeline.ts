 ...
 *
 * This is the "first touch" — the system prompt that shapes every interaction.
 * Currently Brain's ai-router.ts NEVER populates systemPrompt. This service
 * builds what SHOULD be sent, and exposes it for admin visualization.
 *
...
// Porter's system prompt per gateway — what Porter tells the model about itself
// These need proper research and refinement. Starting with honest, minimal prompts.
...
export async function buildGatewayPromptProfile(gatewayType: string, gatewayName: string): Promise<GatewayPromptProfile> {
  ...
  // Layer 1: Porter's system prompt for this gateway
  ...
  // Layer 2: Directives (workspace rules that apply to all dispatch)
...
// ── Build profiles for all gateways ───────────────────
