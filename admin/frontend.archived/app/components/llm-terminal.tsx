...
export function LLMTerminal({ lines, title = "LLM Activity", pulse = false, className }: LLMTerminalProps) {
  return (
    <div className={cn(
      ...
      pulse && lines.length > 0 ? "border-accent-porter/40 shadow-[var(--shadow-accent-pulse)]" : "border-border",
      className
    )}>
      {/* Header */}
      <div className="flex items-center gap-2 bg-raised px-3 py-1.5 border-b border-border">
        ...
        <div className="flex items-center gap-1.5 flex-1">
          ...
          <span className="text-2xs font-mono text-text3">{title}</span>
        </div>
        <span className="flex items-center gap-1 text-2xs text-success font-mono">
