import{w as h,x as f,a as v,p as e}from"./chunk-LFPYN7LY-BE_YIfuT.js";import{A as b}from"./arrow-left-BJ3fvT4W.js";import"./createLucideIcon-Bx7t8UCt.js";const w=`## v6.31.2 (2026-06-13)
...
`;function k(l){const r=[],i=l.split(/^## /m).filter(Boolean);for(const t of i){const a=t.trim().split(`
`),s=(a[0]||"").match(/^(?:(Admin|Brain)\s+)?(v[\d.]+)\s*\((\d{4}-\d{2}-\d{2})\)/);if(!s)continue;const u=(s[1]?`${s[1]} `:"")+s[2],c=s[3],g=new Date(c+"T00:00:00").toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"2-digit"});let p="";const m=[];for(let o=1;o<a.length;o++){const n=a[o].trim();if(n){if(n.startsWith("**")&&n.endsWith("**")){p=n.replace(/\*\*/g,"");continue}if(n.startsWith("- ")){m.push(n.slice(2));continue}}}r.push({version:u,date:g,rawDate:c,highlight:p,items:m})}return r}const C=h(function(){const r=f(),i=v.useMemo(()=>k(w),[]);return e.jsx("div",{className:"flex-1 overflow-y-auto scrollbar-thin p-4",children:e.jsxs("div",{className:"max-w-2xl space-y-3 p-4",children:[e.jsxs("button",{onClick:()=>r(-1),className:"flex items-center gap-1.5 text-xs text-text3 hover:text-text2 transition-colors mb-2",children:[e.jsx(b,{className:"size-3.5"})," Back"]}),i.length===0?e.jsx("p",{className:"text-xs text-text3",children:"No changelog entries"}):i.map(t=>e.jsxs("div",{className:"rounded-xl border border-border bg-surface p-4",children:[e.jsxs("div",{className:"flex items-center justify-between mb-2",children:[e.jsx("h2",{className:"text-sm font-bold text-text",children:t.version}),e.jsx("span",{className:"text-2xs text-text3 font-mono",children:t.date})]}),t.highlight&&e.jsx("p",{className:"text-xs font-medium text-accent-porter mb-2",children:t.highlight}),t.items.length>0&&e.jsx("div",{className:"space-y-1",children:t.items.map((a,d)=>e.jsxs("p",{className:"text-xs text-text2",children:["• ",a]},d))})]},t.version))]})})});export{C as default};
