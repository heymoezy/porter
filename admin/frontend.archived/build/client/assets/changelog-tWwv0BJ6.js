import{w as g,x as f,a as x,p as e}from"./chunk-LFPYN7LY-BE_YIfuT.js";import{A as v}from"./arrow-left-BJ3fvT4W.js";import"./createLucideIcon-Bx7t8UCt.js";const w=`## v6.54.0 (2026-07-07)
...
`;function b(l){const o=[],i=l.split(/^## /m).filter(Boolean);for(const t of i){const a=t.trim().split(`
`),n=(a[0]||"").match(/^(?:(Admin|Brain)\s+)?(v[\d.]+)\s*\((\d{4}-\d{2}-\d{2})\)/);if(!n)continue;const p=(n[1]?`${n[1]} `:"")+n[2],d=n[3],u=new Date(d+"T00:00:00").toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"2-digit"});let h="";const m=[];for(let r=1;r<a.length;r++){const s=a[r].trim();if(s){if(s.startsWith("**")&&s.endsWith("**")){h=s.replace(/\*\*/g,"");continue}if(s.startsWith("- ")){m.push(s.slice(2));continue}}}o.push({version:p,date:u,rawDate:d,highlight:h,items:m})}return o}const B=g(function(){const o=f(),i=x.useMemo(()=>b(w),[]);return e.jsx("div",{className:"flex-1 overflow-y-auto scrollbar-thin p-4",children:e.jsxs("div",{className:"max-w-2xl space-y-3 p-4",children:[e.jsxs("button",{onClick:()=>o(-1),className:"flex items-center gap-1.5 text-xs text-text3 hover:text-text2 transition-colors mb-2",children:[e.jsx(v,{className:"size-3.5"})," Back"]}),i.length===0?e.jsx("p",{className:"text-xs text-text3",children:"No changelog entries"}):i.map(t=>e.jsxs("div",{className:"rounded-xl border border-border bg-surface p-4",children:[e.jsxs("div",{className:"flex items-center justify-between mb-2",children:[e.jsx("h2",{className:"text-sm font-bold text-text",children:t.version}),e.jsx("span",{className:"text-2xs text-text3 font-mono",children:t.date})]}),t.highlight&&e.jsx("p",{className:"text-xs font-medium text-accent-porter mb-2",children:t.highlight}),t.items.length>0&&e.jsx("div",{className:"space-y-1",children:t.items.map((a,c)=>e.jsxs("p",{className:"text-xs text-text2",children:["• ",a]},c))})]},t.version))]})})});export{B as default};
