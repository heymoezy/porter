      ...
      <div className="flex items-center gap-2">
        ...
        {tab === "feed" && (
          <div className="ml-auto flex items-center gap-1.5">
            {actionFilter && (
              <Badge className="text-[10px] bg-accent-porter/15 text-accent-porter border-0 cursor-pointer" onClick={() => setActionFilter("")}>
                {actionFilter} ×
              </Badge>
            )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/50 text-left">
                    <th className="px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-text3">Time</th>
                    <th className="px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-text3">Actor</th>
                    <th className="px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-text3">Action</th>
                    <th className="px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-text3">Target</th>
                  </tr>
                </thead>
                <tbody>
                  {feedData.entries.map(e => (
                    <tr key={e.id} className="border-b border-border/20 last:border-0">
                      <td className="px-3 py-1 text-[11px] text-text3 whitespace-nowrap">{fmtRel(e.ts)}</td>
                      <td className="px-3 py-1 text-xs font-medium text-text">{e.actor}</td>
                      <td className="px-3 py-1">
                        <Badge className={`text-[10px] border-0 ${actionColors[e.action] || "bg-text3/15 text-text3"}`}>
                          {e.action}
                        </Badge>
                      </td>
              ...
              {learnData.learnings.map(l => (
                <div key={l.sessionId} className="px-3 py-2">
                  <div className="flex items-center gap-2 mb-1">
                    <Badge className="text-[10px] bg-accent-porter/15 text-accent-porter border-0">{l.source}</Badge>
                    <span className="text-[10px] text-text3">{fmtRel(l.extractedAt)}</span>
                  </div>
