  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-4 gap-2">
        <Card className="border-border bg-surface">
          <CardContent className="flex items-center gap-2 p-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-text3">Open errors</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-border bg-surface">
          <CardContent className="flex items-center gap-2 p-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-text3">Last 24h</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-border bg-surface">
          <CardContent className="p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-text3 mb-2">By Severity</p>
            <div className="flex gap-2">
              {stats.bySeverity.critical > 0 && <Badge className="bg-danger/15 text-danger border-0 text-[10px]">{stats.bySeverity.critical} critical</Badge>}
              {stats.bySeverity.error > 0 && <Badge className="bg-warning/15 text-warning border-0 text-[10px]">{stats.bySeverity.error} error</Badge>}
              {stats.bySeverity.warning > 0 && <Badge variant="outline" className="text-[10px]">{stats.bySeverity.warning} warn</Badge>}
        ...
        <Card className="border-border bg-surface">
          <CardContent className="p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-text3 mb-2">By Source</p>
            <div className="flex gap-2">
              {stats.bySource.client_js > 0 && <Badge variant="outline" className="text-[10px]">{stats.bySource.client_js} frontend</Badge>}
              {stats.bySource.server_api > 0 && <Badge variant="outline" className="text-[10px]">{stats.bySource.server_api} API</Badge>}
              {stats.bySource.agent_error > 0 && <Badge variant="outline" className="text-[10px]">{stats.bySource.agent_error} agent</Badge>}
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="border-border bg-surface">
          <CardContent className="p-3">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-[11px] font-semibold uppercase tracking-wide text-text3">Recurring Errors</h3>
            ...
            <div className="space-y-1.5">
              {stats.topErrors.map((e, i) => (
                <div key={i} className="flex items-center gap-2 rounded-lg bg-background px-3 py-2">
                  <Badge className={`text-[10px] ${severityColor[e.severity] ?? "bg-text3/15 text-text3"} border-0`}>
                    {e.severity}
                  </Badge>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-text truncate">{e.message}</p>
                    <p className="text-[10px] text-text3">{e.source} · {fmtRel(e.last_seen)}</p>
                  </div>
                  <Badge variant="outline" className="text-[10px] shrink-0">{e.count}x</Badge>
      ...
      <Card className="border-border bg-surface">
        <CardContent className="p-3">
          <h3 className="text-[11px] font-semibold uppercase tracking-wide text-text3 mb-2">Error Log</h3>
          {errors.length === 0 ? (
                ...
                return (
                  <div key={e.id}>
                    ...
                    >
                      ...
                      <Badge className={`text-[10px] ${severityColor[e.severity] ?? ""} border-0 shrink-0`}>
                        {e.severity}
                      </Badge>
                      <p className="text-xs text-text flex-1 min-w-0 truncate">{e.message}</p>
                      {e.username && <span className="text-[10px] text-text3 shrink-0">@{e.username}</span>}
                      <span className="text-[10px] text-text3 shrink-0">{fmtRel(e.created_at)}</span>
                      <Button variant="ghost" size="icon-xs" onClick={(ev) => { ev.stopPropagation(); resolve.mutate(e.id) }}>
                        <X className="size-3" />
                      </Button>
                    </div>
                    {isExpanded && e.stack && (
                      <pre className="mx-3 mt-1 mb-2 overflow-x-auto rounded-md bg-background p-3 text-[11px] text-text3 font-mono leading-relaxed">
                        {e.stack}
                      </pre>
                    )}
                  </div>
