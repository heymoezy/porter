#!/usr/bin/env python3
"""Porter v0.12.59 — self-hosted file manager"""
...
.node-hdr .btn { flex-shrink:0; height:22px; padding:0 7px !important; border-radius:6px; }
...
<aside class="sidebar">
  ...
  <div class="sidebar-footer">
    <div style="font-size:10px;color:var(--text3);margin-bottom:12px;letter-spacing:0.5px">PORTER v0.12.59</div>
  </div>
</aside>
...
<main class="main" id="mainEl">
  ...
  <div id="tasks-module" class="module-panel">
    <div class="module-hdr">
      ...
      <button class="btn btn-ghost" style="font-size:12px" onclick="openTaskWizard()">Task Wizard</button>
    ...
    <div id="tasks-wizard" style="display:none;margin-bottom:12px;background:var(--raised);border:1px solid var(--border);border-radius:8px;padding:10px 12px">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:8px">
        <div style="font-size:12px;font-weight:600;color:var(--text)">Task Wizard</div>
        <button class="btn btn-ghost" style="font-size:11px;padding:3px 8px" onclick="closeTaskWizard()">Close</button>
      </div>
      <div style="font-size:12px;color:var(--text3);margin-bottom:8px">Tell Porter what you want to do and get exact next steps.</div>
      <div style="display:grid;grid-template-columns:180px 1fr auto;gap:8px;align-items:end;margin-bottom:8px">
        <div>
          <div style="font-size:10px;color:var(--text3);margin-bottom:4px">Goal</div>
          <select id="tw-goal" class="settings-input" style="height:32px">
            <option value="unblock">Unblock stuck work</option>
            <option value="stabilize">Stabilize active runs</option>
            <option value="cleanup">Clean finished queue</option>
            <option value="start">Start fresh work</option>
          </select>
        </div>
        <div>
          <div style="font-size:10px;color:var(--text3);margin-bottom:4px">Context</div>
          <input id="tw-context" class="settings-input" style="height:32px" placeholder="Optional: what are you trying to accomplish?">
        </div>
...
function openTaskWizard() {
  const w = document.getElementById('tasks-wizard');
...
function runTaskWizard() {
  const goal = document.getElementById('tw-goal')?.value || 'unblock';
  const ctx = (document.getElementById('tw-context')?.value || '').trim();
  const out = document.getElementById('tw-output');
  if (!out) return;
  const plans = {
    unblock: [
      '1) Open **Needs action** and take the first stalled item.',
      '2) Click **Recover**. If it fails twice, click **Cancel** and re-run clean.',
      '3) Confirm stalled count drops to 0.'
    ],
    stabilize: [
      '1) Review **In progress** for long-running tasks.',
      '2) Pause only if resources are constrained.',
      '3) Resume in priority order and monitor heartbeat freshness.'
    ],
    cleanup: [
      '1) Verify completed outputs are acceptable.',
      '2) Click **Clear completed** to reduce noise.',
      '3) Keep only actionable tasks in the queue.'
    ],
    start: [
      '1) Ensure queue has no critical blockers.',
      '2) Go to the source module and create/start a new task.',
      '3) Return to Tasks and watch it appear under **In progress**.'
    ],
  };
  const hdr = goal === 'unblock' ? 'Unblock plan' : goal === 'stabilize' ? 'Stabilization plan' : goal === 'cleanup' ? 'Cleanup plan' : 'Start-work plan';
  out.innerHTML = `<div style="font-weight:600;color:var(--text);margin-bottom:6px">${hdr}</div><div style="line-height:1.5">${plans[goal].map(p=>`<div>${p}</div>`).join('')}</div>${ctx ? `<div style="margin-top:8px;color:var(--text3)">Context noted: ${escHtml(ctx)}</div>` : ''}`;
}
...
if __name__ == "__main__":
    ...
    print(f"\n  Porter v0.12.59 ready (localhost only)")
