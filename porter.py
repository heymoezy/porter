#!/usr/bin/env python3
"""Porter v0.12.14 — self-hosted file manager"""
...
<aside class="sidebar">
  ...
  <div class="sidebar-footer">
    <div style="font-size:10px;color:var(--text3);margin-bottom:12px;letter-spacing:0.5px">PORTER v0.12.14</div>
  </div>
</aside>
  ...
  targets.forEach(el => {
    ...
    nodes.forEach(node => {
      ...
      if (!mounts.length) {
        ...
        empty.onclick = () => { switchModule('locations'); openAddMount(node.id); };
...
if __name__ == "__main__":
    ...
    print(f"\n  Porter v0.12.14 ready (localhost only)")
