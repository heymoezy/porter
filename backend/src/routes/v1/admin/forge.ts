...
export default async function forgeRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/', async () => {
    ...
    const items = (await pool.query(`
      SELECT fp.*,
        COALESCE(at.name, INITCAP(REPLACE(fp.template_id, '_', ' '))) as template_name,
        at.category
  ...
  fastify.get('/wave-summary', async () => {
    ...
    const summary = (await pool.query(`
      SELECT
        ...
        SUM(CASE WHEN status = 'error' OR status = 'dead_letter' THEN 1 ELSE 0 END) as errors,
  ...
  fastify.delete('/:id', async (request, reply) => {
    ...
    const item = (await pool.query("SELECT id FROM forge_pipeline WHERE id = $1", [id])).rows[0];
    if (!item) {
      reply.status(404);
      return err('NOT_FOUND', 'Item not found');
    }
    await pool.query("DELETE FROM forge_station_runs WHERE pipeline_id = $1", [id]);
    await pool.query("DELETE FROM forge_pipeline WHERE id = $1", [id]);
