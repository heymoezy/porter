...
export default async function systemRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/', async () => {
    ...
    let dbSize = 0;
    try {
      const pgSize = await pool.query("SELECT pg_database_size(current_database()) as size");
      dbSize = parseInt(pgSize.rows[0].size) || 0;
    } catch {}

    // Active sessions (valid tokens)
    let activeSessions = 0;
    try {
      activeSessions = (await pool.query("SELECT count(*) as c FROM sessions WHERE expires > EXTRACT(EPOCH FROM NOW())")).rows[0].c;
    } catch {}

    // Concurrent users — distinct users active in last 5 minutes
    let concurrentUsers = 0;
    try {
      concurrentUsers = (await pool.query("SELECT count(DISTINCT username) as c FROM sessions WHERE last_seen_at > EXTRACT(EPOCH FROM NOW()) - 300")).rows[0].c;
    } catch {}
