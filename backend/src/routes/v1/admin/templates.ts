...
export default async function templatesRoutes(fastify: FastifyInstance) {
  ...
  // GET /api/admin/templates — list all templates from DB
  fastify.get('/', async (req) => {
    ...
    const templates = (rows as TemplateRow[]).map(formatTemplate);
  ...
  fastify.get('/stats', async () => {
    const { rows } = await pool.query(`
      SELECT category, archetype, COUNT(*) as count
    ...
    for (const row of rows as Array<{ category: string; archetype: string; count: string }>) {
  ...
  fastify.get('/:id', async (req) => {
    ...
    const row = rows[0] as TemplateRow & Record<string, unknown>;
