...
export default async function settingsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/', async () => {
    // General
    const totalUsers = (await pool.query('SELECT count(*) as cnt FROM users')).rows[0]?.cnt ?? 0;
    ...
    try {
      activeSessions = (await pool.query("SELECT count(*) as cnt FROM sessions WHERE expires_at > EXTRACT(EPOCH FROM NOW())")).rows[0]?.cnt ?? 0;
    } catch {
      try {
        activeSessions = (await pool.query("SELECT count(*) as cnt FROM sessions WHERE expires > $1", [Date.now() / 1000])).rows[0]?.cnt ?? 0;
  ...
  fastify.post('/force-logout', async (req) => {
    const currentToken = req.cookies?.porter_session;
    let terminated = 0;
    try {
      if (currentToken) {
        const result = await pool.query("DELETE FROM sessions WHERE token != $1", [currentToken]);
      ...
      // sessions table might use different schema
      try {
        if (currentToken) {
          const result = await pool.query("DELETE FROM sessions WHERE session_token != $1", [currentToken]);
          terminated = result.rowCount ?? 0;
        } else {
          const result = await pool.query("DELETE FROM sessions");
          terminated = result.rowCount ?? 0;
