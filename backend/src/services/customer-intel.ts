...
export async function computeScores(username: string) {
  ...
  const user = (await pool.query('SELECT created_at FROM users WHERE username = $1', [username])).rows[0] as any;
  if (!user) return null;
  ...
  const projects = (await pool.query(
    'SELECT COUNT(*) as c FROM projects WHERE owner_id = $1', [username]
  )).rows[0].c;

  const chats = (await pool.query(
    'SELECT COUNT(*) as c FROM chats WHERE username = $1', [username]
  )).rows[0].c;

  const agents = (await pool.query(
    'SELECT COUNT(*) as c FROM personas WHERE owner = $1 AND is_system = false', [username]
  )).rows[0].c;
  ...
  try {
    loginCount = (await pool.query(
      "SELECT COUNT(*) as c FROM customer_events WHERE username = $1 AND event_type = 'login' AND created_at > $2",
  ...
  let shareCount = 0;
  try {
    shareCount = (await pool.query(
      "SELECT COUNT(*) as c FROM customer_events WHERE username = $1 AND event_type = 'share'", [username]
    )).rows[0].c;
  } catch {}
  ...
  try {
    invitesSent = (await pool.query(
      'SELECT COUNT(*) as c FROM invites WHERE created_by = $1', [username]
    )).rows[0].c;
    invitesConverted = (await pool.query(
      'SELECT COUNT(*) as c FROM invite_uses iu JOIN invites iv ON iu.invite_code = iv.code WHERE iv.created_by = $1', [username]
    )).rows[0].c;
  } catch {}
...
export async function getAgentTasks(limit = 20) {
  try {
    return (await pool.query(
      'SELECT * FROM admin_agent_tasks ORDER BY priority DESC, created_at DESC LIMIT $1', [limit]
    )).rows as any[];
  } catch { return []; }
}
...
export async function getAgentTaskStats() {
  try {
    const queued = (await pool.query("SELECT COUNT(*) as c FROM admin_agent_tasks WHERE status = 'queued'")).rows[0].c;
    const running = (await pool.query("SELECT COUNT(*) as c FROM admin_agent_tasks WHERE status = 'running'")).rows[0].c;
    const completed = (await pool.query("SELECT COUNT(*) as c FROM admin_agent_tasks WHERE status = 'completed'")).rows[0].c;
...
export async function getLoginHistory(username: string) {
  try {
    return (await pool.query(`
      ...
      WHERE username = $1 AND event_type = 'login'
      ORDER BY created_at DESC LIMIT 50
    `, [username])).rows as any[];
  } catch { return []; }
}
...
export async function getLoginAnomalies(username: string) {
  ...
  try {
    ...
    const recentBurst = (await pool.query(`
      SELECT COUNT(*) as c FROM customer_events
      WHERE username = $1 AND event_type = 'login' AND created_at > $2
    `, [username, Date.now() / 1000 - 3600])).rows[0] as any;
    if (recentBurst.c > 20) {
    ...
    const recentIps = (await pool.query(`
      ...
      WHERE username = $1 AND event_type = 'login' AND created_at > $2
    `, [username, Date.now() / 1000 - 86400])).rows[0] as any;
    if (recentIps.c > 5) {
...
export async function backfillLoginEvents() {
  try {
    const existing = (await pool.query("SELECT COUNT(*) as c FROM customer_events WHERE event_type = 'login'")).rows[0] as any;
    ...
    for (const s of sessions) {
      ...
      seen.add(key);
      await pool.query(
        "INSERT INTO customer_events (username, event_type, ip_address, created_at) VALUES ($1, 'login', $2, $3)",
        [s.username, s.ip_address, s.created_at]
      );
    }
