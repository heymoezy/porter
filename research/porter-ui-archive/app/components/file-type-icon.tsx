...
const labelSize = { sm: "text-[7px]", md: "text-[8px]", lg: "text-[10px]" }
...
const CATEGORY_STYLE: Record<FileCategory, { bg: string; fg: string; label: string; icon: (s: number) => ReactNode }> = {
  ...
  video: {
    bg: "bg-danger/12", fg: "text-danger", label: "VID",
  ...
  audio: {
    bg: "bg-chart-3/12", fg: "text-chart-3", label: "AUD",
    ...
    <div className="flex flex-wrap gap-3">
      {files.map(f => (
        <div key={f} className="flex flex-col items-center gap-1.5">
          ...
          <span className="text-[10px] text-text3 truncate max-w-[56px]">{f}</span>
        </div>
      ))}
    </div>
  )
